...
class LoginControl {
   ...
   static async esqueciSenha(req, res){
       ...
       try {
            ...
            user.resetSenha = token

            await user.save()

            EmailController.sendMail({
                to: email,
                from: 'equipe@stacktion.com',
                subject: 'Esqueceu sua senha? - Stacktion',
   ...
   static async resetarSenha(req, res) {
        ...
        try {
            const user = await database.users.findOne({where:{email: email, resetSenha: token}})
